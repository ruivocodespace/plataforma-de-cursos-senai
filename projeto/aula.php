        ...
        <aside class="w-72 bg-gray-800 border-r border-gray-700 flex-shrink-0 overflow-y-auto hidden lg:block" style="height: calc(100vh - 44px); position: sticky; top: 44px;">
            <div class="p-4">
                ...
                while ($mod = mysqli_fetch_assoc($lista_modulos)) { 
                    ...
                    $cor_icone_mod = ($id_deste_modulo == $modulo_id) ? "bg-senai-blue" : "bg-gray-600";
                ?>
                    <div class="mb-4 <?= $opacidade ?>">
                        ...
                        <ul class="space-y-1 pl-2">
                            ...
                            while ($aul = mysqli_fetch_assoc($lista_aulas)) {
                                ...
                                $chk_aula = mysqli_query($conexao, "SELECT id FROM progresso WHERE usuario_id = $usuario_id AND aula_id = $id_desta_aula AND concluido = 1");
                                $aula_feita = mysqli_num_rows($chk_aula) > 0;

                                if ($id_desta_aula == $aula_id) {
                                    // Aula Atual
                                    echo '<a href="aula.php?id='.$id_desta_aula.'" class="flex items-center gap-2 py-1.5 px-2 rounded bg-senai-blue text-xs text-white cursor-pointer block">';
                                    echo '<span class="w-4 h-4 bg-white/30 rounded-full flex items-center justify-center flex-shrink-0" style="font-size:9px">▶</span>';
                                    echo '<span class="font-semibold">' . $aul['titulo'] . '</span>';
                                    echo '<span class="ml-auto text-blue-200">'.$duracao_txt.'</span>';
                                    echo '</a>';
                                } else if ($aula_feita) {
                                    // Aula Concluída
                                    echo '<a href="aula.php?id='.$id_desta_aula.'" class="flex items-center gap-2 py-1.5 px-2 rounded text-xs text-green-400 cursor-pointer hover:bg-gray-700 block">';
                                    ...
                                    echo '<span class="ml-auto text-gray-500">'.$duracao_txt.'</span>';
                                    echo '</a>';
                                } else {
                                    // Aula Pendente
                                    echo '<a href="aula.php?id='.$id_desta_aula.'" class="flex items-center gap-2 py-1.5 px-2 rounded text-xs text-gray-400 hover:bg-gray-700 block">';
                                    echo '<span class="w-4 h-4 bg-gray-700 rounded-full flex-shrink-0"></span>';
                                    echo '<span>' . $aul['titulo'] . '</span>';
                                    echo '<span class="ml-auto text-gray-600">'.$duracao_txt.'</span>';
                                    echo '</a>';
                                }
                            }
                            ?>
            ...
            <div class="bg-white p-6 lg:p-8">
                ...
                <div class="flex items-start justify-between gap-4 mb-4">
                    <div>
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="text-gray-400 text-xs">⏱ <?= !empty($aula['duracao']) ? $aula['duracao'] : '--:--' ?></span>
                        </div>
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        $lista_aulas_rodape = mysqli_query($conexao, "SELECT * FROM aulas WHERE modulo_id = $modulo_id ORDER BY ordem ASC");
                        
                        while ($aul_rodape = mysqli_fetch_assoc($lista_aulas_rodape)) {
                            $id_r = $aul_rodape['id'];
                            $dur = !empty($aul_rodape['duracao']) ? $aul_rodape['duracao'] : "--:--";
                            
                            $chk_r = mysqli_query($conexao, "SELECT id FROM progresso WHERE usuario_id = $usuario_id AND aula_id = $id_r AND concluido = 1");
                            ...
                            if ($id_r == $aula_id) {
                                ...
                                echo '<span class="text-sm font-semibold text-gray-800">' . $aul_rodape['titulo'] . '</span>';
                                echo '<span class="ml-auto text-xs text-gray-400">'.$dur.'</span>';
                                ...
                                echo '<span class="text-sm text-gray-600">' . $aul_rodape['titulo'] . '</span>';
                                echo '<span class="ml-auto text-xs text-gray-400">'.$dur.'</span>';
                                echo '<a href="aula.php?id='.$id_r.'" class="text-xs text-senai-blue underline ml-2">Rever</a>';
                                echo '</div>';
                            } else {
                                // Pendente
                                ...
                                echo '<span class="text-sm text-gray-600">' . $aul_rodape['titulo'] . '</span>';
                                echo '<span class="ml-auto text-xs text-gray-400">'.$dur.'</span>';
                                echo '<a href="aula.php?id='.$id_r.'" class="text-xs text-senai-blue underline ml-2">Assistir</a>';
                                echo '</div>';
                            }
                        }
                        ?>
